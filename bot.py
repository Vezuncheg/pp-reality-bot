...
async def schedule_dojim(uid, context):
    ...
    async def block2(ctx):
        ...
        await ctx.bot.send_message(uid,
            ...
            "Старт *6 июля*. Если урок откликнулся — самое время сделать следующий шаг.",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("Записаться в Реалити →", url=PAY_URL)]
            ])
...
async def schedule_next_unseen(uid, current_block, context_or_jq):
    ...
    if app and hasattr(app, 'user_data'):
        raw = app.user_data.get(uid, {})
        # user_data может быть mappingproxy — копируем в обычный set
        raw_sent = raw.get("blocks_sent", set()) if hasattr(raw, 'get') else set()
        sent = set(raw_sent)  # гарантируем изменяемый set
    else:
        sent = set()
    ...
    if app and hasattr(app, 'user_data'):
        if uid not in app.user_data:
            ...
            try:
                ...
                # mappingproxy — не можем записать, игнорируем
                pass
