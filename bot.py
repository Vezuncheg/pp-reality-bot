...
async def schedule_dojim(uid, context):
    ...
    async def block1(ctx):
        ...
        await ctx.bot.send_message(
            uid,
            "*Иван Самохин. 38 лет. 24 года в зале.*\n\n"
            "В детстве — слабый, болезненный ребёнок с плохим иммунитетом. "
            "Те, кто видит его сейчас, говорят: «Ну у тебя генетика». "
            "Если бы они видели его в 14 лет — дрыщ, который не мог набрать массу. "
            "Генетика здесь ни при чём.\n\n"
            "С тех пор прошло 24 года. И нет — он не тот человек, который никогда не срывался "
            "и всегда был в форме. Бывало по-разному.\n\n"
            "Ломал ногу — вылетал на несколько месяцев.\n"
            "Много работал, поздно уходил из офиса, постоянно ездил в командировки.\n"
            "Форма уходила. Жизнь брала своё.\n\n"
            "Именно в этом контексте *обычной жизни* он и научился находить время для себя. "
            "Выработал принципы и привычки, которые стали опорой — не железной волей, "
        ...
        await ctx.bot.send_message(
            uid,
            "*Почему ему можно доверять — и почему это не диплом*\n\n"
            "В 2024 году Иван начал вести подкаст. "
            "За это время — больше 60 выпусков с учёными, докторами, генетиками, "
            "эндокринологами, биофизиками.\n\n"
            "Подкаст — это не диплом, он понимает. "
            "Но чтобы подготовиться к каждому интервью, нужно глубоко погрузиться в тему. "
            "Бывало, что в ходе разговора он понимал: сам доктор заблуждается — "
            "потому что появились новые данные, которые опровергают то, во что тот верил годами.\n\n"
        ...
        await ctx.bot.send_message(
            uid,
            "*«Я не тренер и не врач — и считаю, что это плюс»*\n\n"
            "Часто можно увидеть докторов, которые сами выглядят не очень — с лишним весом, "
            "с плохими привычками. Один простой вопрос: почему нужно слушать человека, "
            "который на своём теле не применяет то, о чём говорит?\n\n"
            "С тренерами другая история. За 24 года Иван занимался с большим количеством тренеров "
            "в разных залах и странах. Почти все рано или поздно предлагали перейти на анаболики — "
            "им важно сделать из Вас кейс. При этом мало кто думает о реальном здоровье "
            "и долголетии.\n\n"
            "Иван выбрал другой путь — рациональный. "
            "Жить полноценно, без перегибов, без лотков с едой и зомби-режима. "
            "С фастфудом, ресторанами, командировками. И при этом — выглядеть хорошо.\n\n"
        ...
        await ctx.bot.send_message(
            uid,
            "*Прямо перед запуском программы Иван прошёл её сам.*\n\n"
            "С нуля. Всё зафиксировал. И снял на камеру — день за днём.\n\n"
            "За 28 дней:\n"
            "→ −4,3 кг веса\n"
            "→ −2,2 кг жировой массы\n"
            "→ −1,8% телесного жира\n\n"
            "Он не продаёт мечту про «стань качком за месяц». "
            "Он говорит о принципах и привычках, которые 24 года позволяют выглядеть хорошо "
            "в условиях обычной жизни — с фастфудом, ресторанами и периодами, "
        ...
        await ctx.bot.send_message(
            uid,
            "Завтра расскажу подробно — что именно Вас ждёт внутри Реалити и как это устроено.",
            reply_markup=InlineKeyboardMarkup([
